...
<body>
    <div class="container mt-5">
        <?php
        include('./db.php'); // подключаем и устанавливаем соединение с БД
        if (!empty($_POST["send"])) {
            $name = $_POST["name"];
            $email = $_POST["email"];
            $phone = $_POST["phone"];
            ...
            if (mail($toMail, $subject, $message, $header)) {
                ...
                $sql = "INSERT INTO contacts_list(name, email, phone, subject, message, sent_date) VALUES (?,?,?,?,?,?)";
                if (!$sql) {
        ...
        <?php } ?>
        <!-- Contact form -->
        <form action="./contact-form.php" name="contactForm" method="post" enctype="multipart/form-data">
